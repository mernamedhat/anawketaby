...
class QuestionOrderingDialog extends StatefulWidget {
  final Competition competition;
  final CompetitionQuestion? competitionQuestion;

  const QuestionOrderingDialog(
      {Key? key, required this.competition, this.competitionQuestion})
      : super(key: key);

  @override
  _QuestionOrderingDialogState createState() => _QuestionOrderingDialogState();
}

class _QuestionOrderingDialogState extends State<QuestionOrderingDialog> {
  ...
  void _refreshChoices() {
    _constantChoices.clear();
    for (QuestionChoice choice in _choices) {
      choice.no = _choices.indexOf(choice) + 1;
      _constantChoices.add(QuestionChoice(choice.no, "${choice.no}"));
    }
  }
